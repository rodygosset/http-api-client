...
- `url: U` - URL endpoint (string or function)
- `method: M` - HTTP method
- `headers?: H` - Optional headers (static or function)
- `body?: I` - Optional request body schema
- `response?: O` - Optional response handler (Schema or function)
- `error?: E` - Optional error handler (Schema or function)
- `filterStatusOk?: boolean` - Filter non-OK status codes (default: `false`)

### Convenience Methods

#### `get<U, H, O, E>(spec)`

Creates a GET request handler. Body is not allowed.

```ts
const getTodo = HttpApiClient.get({
  url: (params: { id: string }) => `/todos/${params.id}`,
  headers: HeadersFunction,
  response: OutputFunction,
  error: ErrorFunction,
...
Creates a POST request handler with optional body.
...
Creates a PUT request handler with optional body.

#### `del<U, H, I, O, E>(spec)`

Creates a DELETE request handler with optional body.

### Client Class

The `Client` class provides a way to define default headers and error handling that are automatically applied to all routes created from it.
...
- `headers?: DefaultHeaders` - Optional default headers applied to all routes. Can be a static `Headers.Headers` instance or a function that returns an `Effect<Headers>`.
- `error?: DefaultError` - Optional default error handler applied to all routes. Can be a Schema for automatic parsing or a function that transforms the error response.

**Methods:**

#### `client.get<U, H, O, E>(spec)`

Creates a GET request handler that inherits the client's default headers and error handler.
...
const getTodo = apiClient.get({
  url: (params: { id: string }) => `/todos/${params.id}`,
  headers: HeadersFunction, // Optional: overrides default headers
  response: OutputFunction,
  error: ErrorFunction, // Optional: overrides default error handler
  filterStatusOk: boolean,
});
```

#### `client.post<U, H, I, O, E>(spec)`

Creates a POST request handler that inherits the client's default headers and error handler.
...
#### `client.put<U, H, I, O, E>(spec)`

Creates a PUT request handler that inherits the client's default headers and error handler.
...
#### `client.del<U, H, I, O, E>(spec)`

Creates a DELETE request handler that inherits the client's default headers and error handler.

```ts
const deleteTodo = apiClient.del({
  url: (params: { id: string }) => `/todos/${params.id}`,
  headers: HeadersFunction,
  body: Schema.Schema<any>, // Rarely used for DELETE
  response: OutputFunction,
  error: ErrorFunction,
  filterStatusOk: boolean,
});
...
- Routes automatically inherit default headers and error handlers from the client
- Routes can override defaults by specifying their own `headers` or `error` properties
- Type-safe defaults flow through to all route methods
- Multiple clients can be created for different API services or environments
...
```ts
type UrlFunction = string | ((arg: any) => string);
```
...
Context tag for API configuration:

```ts
class Config extends Context.Tag("@HttpApiClient/Config")<
  Config,
...
Layer that provides a configured `HttpClient`:
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // ... configures HttpClient with base URL and auth
  })
);
```
...
- `HttpClient.HttpClient` - Base HTTP client (e.g., `FetchHttpClient.layer`)

## Real-World Examples

### Example: Document Download with Content Type Detection
  ...
  response: (res: HttpClientResponse.HttpClientResponse) =>
    Effect.gen(function* () {
      const contentType = isPDF
        ? "application/pdf"
        : Headers.get("Content-Type")(res.headers).pipe(
            Option.getOrElse(() => "application/octet-stream")
          );
...
### Example: Type-Safe Enum Statuses

```ts
const statuses = ["pending", "in-progress", "completed", "cancelled"] as const;

const getTodoStatuses = HttpApiClient.get({
  url: (params: { todoId: string }) => `/todos/${params.todoId}/statuses`,
  response: Schema.Literal(...statuses).pipe(Schema.Array),
});

// Return type is automatically inferred as Array<"pending" | "in-progress" | "completed" | "cancelled">
const program = Effect.gen(function* () {
  const todoStatuses = yield* getTodoStatuses({
    url: { todoId: "123" },
  });
  return todoStatuses;
});
...
1. **Use Schema for all data structures** - Ensures runtime validation and type safety

2. **Leverage Effect composition** - Chain multiple API calls using `Effect.gen` for readable async code

3. **Use Layers for configuration** - Makes your code testable and flexible

4. **Handle errors explicitly** - Use Effect's error channel with `catchAll` or `catchTag`

5. **Use convenience methods** - Prefer `get()`, `post()`, etc. over `make()` for better type inference

6. **Document your schemas** - Use Schema's metadata features to document your API contracts
